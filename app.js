...
(() => {
  ...
  dockNode.addEventListener('pointerdown', (event) => { const item = event.target.closest('.dock-item[data-id]'); if (!item) return; event.preventDefault(); item.setPointerCapture?.(event.pointerId); dockPointer = { id: item.dataset.id, startY: event.clientY, moved: false, pointerId: event.pointerId }; });
  dockNode.addEventListener('pointermove', (event) => { if (!dockPointer || event.pointerId !== dockPointer.pointerId) return; event.preventDefault(); if (Math.abs(event.clientY - dockPointer.startY) > 6) dockPointer.moved = true; const over = document.elementFromPoint(event.clientX, event.clientY)?.closest('.dock-item[data-id]'); dockNode.querySelectorAll('.dock-item.drag-over').forEach((node) => node.classList.remove('drag-over')); if (over && over.dataset.id !== dockPointer.id) over.classList.add('drag-over'); });
