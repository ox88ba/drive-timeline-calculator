...
(() => {
  ...
  let searchTimers = new Map(); let pendingDeleteIndex = null; let activeDockId = null; let dockPointer = null; let dockSuppressClickUntil = 0; let dockToastTimer = null; let mapGesture = null; let routeRebuildVersion = 0; let mapRenderVersion = 0; let amapLoadPromise = null; let amapMap = null; let amapOverlays = []; let mapCenterAction = () => {};
  ...
  function buildShareModel() {
    ...
    const destinations = legs.map((leg, index) => {
      ...
      return {
        ...
        arrival: destination.arrivalTime ? formatDateTime(destination.arrivalTime) : '等待导航数据', departure: destination.hasDepartureDisplay ? formatDateTime(destination.departureTime) : '', stay: destination.stayMinutes > 0 ? formatStay(destination.stayMinutes) : '', meta,
        arrivalMoment: shareMoment(arrivalPhoto), departureMoment: shareMoment(departurePhoto),
        routeFromPrevious: { distance: route ? formatDistance(route.distanceMeters) : '待导航', duration: route ? formatDuration(route.durationSeconds) : '待导航', polyline: Array.isArray(route?.polyline) ? route.polyline.map((point) => [Number(point?.[0]), Number(point?.[1])]).filter(([x, y]) => Number.isFinite(x) && Number.isFinite(y)) : [] }
      };
    });
  ...
  function centerRoutePreview() { if (amapMap && amapOverlays.length) { amapMap.setFitView(amapOverlays, false, [36, 36, 36, 36]); return; } mapCenterAction(); }
  function render() {
  ...
  function resetRefreshChallenge() { const slider = $('#refreshHumanRange'); const confirm = $('#refreshConfirm'); const status = $('#refreshHumanStatus'); if (slider) slider.value = '0'; if (confirm) confirm.disabled = true; if (status) status.textContent = '请将滑块拖到最右侧完成验证'; }
  function openRefreshModal() { resetRefreshChallenge(); $('#refreshModal').hidden = false; }
  function closeRefreshModal() { $('#refreshModal').hidden = true; }
  function updateRefreshChallenge() { const slider = $('#refreshHumanRange'); const confirm = $('#refreshConfirm'); const status = $('#refreshHumanStatus'); const passed = Number(slider?.value || 0) >= 96; if (confirm) confirm.disabled = !passed; if (status) status.textContent = passed ? '验证通过，可以开始重新获取路线' : '请将滑块拖到最右侧完成验证'; }
  function confirmRefresh() { if (Number($('#refreshHumanRange')?.value || 0) < 96) return; closeRefreshModal(); showDockToast('正在重新获取高德道路轨迹…'); rebuildRouteGraph(true); }
