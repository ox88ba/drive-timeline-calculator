...
(() => {
  ...
  const dockShell = document.querySelector('.trip-dock'); const dockToggle = $('#dockToggle');
  ...
  let searchTimers = new Map(); let pendingDeleteIndex = null; let activeDockId = null; let dockPointer = null; let mapGesture = null;
  ...
  function photoFor(location, iso) {
    ...
    let sunsetAt = sunsetCache[key] && sunsetCache[key].sunsetAt;
    if (!sunsetAt) { const calculated = SolarPhotography.sunsetForChinaDate(dateKey, location.latitude, location.longitude); if (!calculated) return null; sunsetAt = calculated.toISOString(); sunsetCache[key] = { sunsetAt }; }
    const deltaMinutes = Math.round((new Date(iso).getTime() - new Date(sunsetAt).getTime()) / 60000);
    return { sunsetAt, deltaMinutes, moment: SolarPhotography.classifyPhotography(deltaMinutes) };
  }
  function calculate() {
  ...
  function updateSummary() { const s = trip.summary; $('#totalDistance').textContent = s.routeChainIsComplete ? formatDistance(s.distanceMeters) : '待导航'; $('#totalDrive').textContent = s.routeChainIsComplete ? formatDuration(s.drivingSeconds) : '待导航'; $('#totalStay').textContent = formatStay(s.totalStayMinutes); $('#totalDuration').textContent = s.routeChainIsComplete ? formatDuration(s.totalDurationSeconds) : '待导航'; $('#finalArrival').textContent = s.finalArrivalTime ? formatDateTime(s.finalArrivalTime, true) : (trip.destinations.length ? '等待完整导航数据' : '添加目的地后计算'); }
  ...
  dockNode.addEventListener('click', (event) => { const item = event.target.closest('.dock-item'); if (!item) return; const anchor = item.dataset.anchor || `destination-${item.dataset.id}`; activeDockId = item.dataset.id || null; renderDock(); document.getElementById(anchor)?.scrollIntoView({ behavior: 'smooth', block: 'center' }); });
  dockNode.addEventListener('pointerdown', (event) => { const item = event.target.closest('.dock-item[data-id]'); if (!item) return; event.preventDefault(); item.setPointerCapture?.(event.pointerId); dockPointer = { id: item.dataset.id, startX: event.clientX, startY: event.clientY, moved: false, pointerId: event.pointerId }; });
  dockNode.addEventListener('pointermove', (event) => { if (!dockPointer || event.pointerId !== dockPointer.pointerId) return; event.preventDefault(); const movedX = Math.abs(event.clientX - dockPointer.startX); const movedY = Math.abs(event.clientY - dockPointer.startY); if (Math.max(movedX, movedY) > 6) dockPointer.moved = true; const over = document.elementFromPoint(event.clientX, event.clientY)?.closest('.dock-item[data-id]'); dockNode.querySelectorAll('.dock-item.drag-over').forEach((node) => node.classList.remove('drag-over')); if (over && over.dataset.id !== dockPointer.id) over.classList.add('drag-over'); });
  dockNode.addEventListener('pointerup', (event) => { if (!dockPointer || event.pointerId !== dockPointer.pointerId) return; event.preventDefault(); const over = document.elementFromPoint(event.clientX, event.clientY)?.closest('.dock-item[data-id]'); const move = dockPointer.moved && over ? [dockPointer.id, over.dataset.id] : null; dockNode.querySelectorAll('.dock-item.drag-over').forEach((node) => node.classList.remove('drag-over')); dockPointer = null; if (move) reorderByIds(...move); });
  dockNode.addEventListener('pointercancel', () => { dockPointer = null; dockNode.querySelectorAll('.dock-item.drag-over').forEach((node) => node.classList.remove('drag-over')); });
