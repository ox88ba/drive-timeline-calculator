...
(async()=>{
 ...
 try {
  ...
  assert.equal(await page.locator('.destination-card .card-more').count(),0,'empty destination has direct delete, not more');
  assert.equal(await page.locator('.card-empty-actions [data-action="remove"]').isVisible(),true);
  ...
  await page.locator('.card-empty-actions [data-action="remove"]').last().click();
  assert.equal(await page.locator('.destination-card').count(),2,'direct delete works while empty search was focused');
  await page.locator('.undo-snackbar-action').click();
  assert.equal(await page.locator('.destination-card').count(),3,'empty destination deletion remains undoable');
  await page.locator('[data-action="add-via-point"]').first().click();
  assert.equal(await page.locator('.destination-card').count(),4);
  assert.equal(await page.locator('.destination-card').first().locator('.card-more').count(),0,'empty waypoint has no more menu');
  await page.locator('.destination-card').first().locator('[data-action="remove"]').click();
  assert.equal(await page.locator('.destination-card').count(),3,'empty waypoint can be deleted directly');
